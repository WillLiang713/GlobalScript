...
GlobalScript 是一份面向 Clash Meta / Mihomo 的精简分流模板。它不追求把每个服务都拆成独立策略组，而是采用更易维护的主线逻辑：**国内与游戏默认直连，其他非中国大陆流量统一进入代理出口，同时保留地区节点选择能力**。
...
也就是说，**除游戏平台外，服务不再拆成独立策略组，但地区出口仍然保留**。如果需要临时切换到香港、日本、美国等地区，只需要在 `代理出口` 中选择对应地区即可。
...
- 游戏相关流量默认直连；如果商店、社区或登录页面无法访问，可以在 `游戏平台` 中临时切到 `代理出口` 或指定地区节点。
