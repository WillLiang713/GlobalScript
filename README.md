...
GlobalScript 是一份面向 Clash Meta / Mihomo 的精简分流模板。它不追求把每个服务都拆成独立策略组，而是采用更易维护的主线逻辑：**国内默认直连，普通国外流量统一进入代理出口，同时为 AI、流媒体、游戏平台保留少量高价值大类入口，并保留地区节点选择能力**。
...
这样可以避免 Google、开发工具、社交通讯等服务各自拆成小组导致的选择负担，同时只为 AI、流媒体、游戏平台保留少量确实有独立出口价值的大类入口。

### 2. 出口选择集中到少量大类

`代理出口` 是普通国外流量和最终兜底的主策略组，并被放在代理组列表最前面，方便在 Zashboard / Yacd / MetaCubeXD 等面板中快速选择。AI、流媒体、游戏平台作为少量高价值大类独立保留。
...
也就是说，**普通国外服务不再拆成独立小组，但 AI、流媒体、游戏平台保留大类入口，地区出口也仍然保留**。如果需要临时切换到香港、日本、美国等地区，只需要在对应大类组或 `代理出口` 中选择对应地区即可。

`代理出口`、`AI出口`、`流媒体`、`游戏平台` 使用统一的候选出口顺序，避免不同策略组之间出现不同排序和不同数量的选项。`自动测速` 是原 `智能优选` 的更准确命名，它只表示根据测试 URL 和延迟进行 `url-test` 自动选择，不代表 IP 质量、AI 可用性或流媒体解锁能力。
...
4. 打开控制面板，在 `代理出口`、`AI出口`、`流媒体` 或 `游戏平台` 中选择自动测速、手动、自建或指定地区出口。
