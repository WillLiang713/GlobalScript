...
const outputDir = path.join(repoRoot, "icons", "adaptive");
...
const plateStyle = `
    :root {
      ...
      --stroke: #d4d4d8;
      --fg: #111827;
      --badge-stroke: rgba(17, 24, 39, 0.12);
      --accent: #f97316;
    }
    @media (prefers-color-scheme: dark) {
      :root {
        ...
        --stroke: #3f3f46;
        --fg: #fafafa;
        --badge-stroke: rgba(250, 250, 250, 0.14);
        --accent: #fb923c;
      }
    }
    .theme-dark {
...
const brandIcons = {
  ...
  "apple.svg": { source: "dashboard", name: "apple", size: 31, dual: true },
  "github.svg": { source: "dashboard", name: "github", size: 31, dual: true },
  ...
  "openai.svg": { source: "dashboard", name: "openai", size: 32, dual: true },
  "telegram.svg": { source: "dashboard", name: "telegram", size: 33 },
  "tiktok.svg": { source: "dashboard", name: "tiktok", size: 32, dual: true },
  "x.svg": { source: "dashboard", name: "x", size: 30, dual: true },
...
function plateShell(title, content) {
...
function makeUtilitySvg(name, { title, body }) {
  const content = `
  <svg x="16" y="16" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="var(--fg)" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
    ${body}
  </svg>`;

  return [`${name}.svg`, plateShell(title, content)];
}

function makeFlagSvg(fileName, _code, { viewBox, inner }) {
  ...
  <rect x="14" y="16" width="36" height="32" rx="8" fill="none" stroke="var(--badge-stroke)" stroke-width="1.25" />`;

  return [fileName, plateShell(title, content)];
}
...
function makeBrandSvg(fileName, source) {
  ...
  if (source.source === "simple") {
    ...
    badge = nestedSvg({
      ...
      y: offset,
      size,
      extra: `${simpleAttrs} fill="currentColor" color="var(--fg)"`.trim(),
    });
  } else if (source.dual && source.light) {
    badge = `
  <g class="theme-light">
    ${nestedSvg({
      viewBox: source.regular.viewBox,
  ...
  <g class="theme-dark">
    ${nestedSvg({
      viewBox: source.light.viewBox,
      body: source.light.inner,
      x: offset,
      y: offset,
      size,
      extra: source.light.outerAttrs,
    })}
  </g>`;
  } else {
    badge = nestedSvg({
  ...
  return [fileName, plateShell(title, badge)];
}
...
async function loadBrandSource(config) {
  ...
  const regular = await fetchSvg(
    `https://raw.githubusercontent.com/homarr-labs/dashboard-icons/main/svg/${config.name}.svg`,
  );
  const light = config.dual
    ? await fetchSvg(
        `https://raw.githubusercontent.com/homarr-labs/dashboard-icons/main/svg/${config.name}-light.svg`,
      )
    : null;

  if (config.name === "openai" && light) {
    light.inner = light.inner.replace(/fill="#ffffff000"/gi, "");
  }

  return { ...config, regular, light };
}
...
async function main() {
  ...
  const flagEntries = await Promise.all(
    Object.entries(flagIcons).map(async ([fileName, code]) => {
      ...
      return makeFlagSvg(fileName, code, svg);
    }),
  );
